...
for (int i = 0; i < CONTACTS_MAX; ++i) {
            if (!contacts_book[i].getFirstName().empty()) {
                std::cout << "| " << std::setw(5) << i + 1 << " | "
                          << std::setw(10) << truncateString(contacts_book[i].getFirstName(), 10) << " | "
                          << std::setw(10) << truncateString(contacts_book[i].getLastName(), 10) << " | "
                          << std::setw(10) << truncateString(contacts_book[i].getNickName(), 10) << " |" << std::endl;
            }
        }
...
//afficher un contact à un index donné
    void PhoneBook::displayContactindice(int index)const {
        while (true) {
            ...
            if (std::cin.eof()) 
                exit(EXIT_FAILURE);
            if (std::cin.fail() || index < 1 || index > CONTACTS_MAX || contacts_book[index -1].getFirstName().empty()) {
                std::cin.clear();
                 std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                std::cout <<RED + "Invalid contact index. Please try again." + RESET << std::endl;
            } else {
                 std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
